...
export function useUpdateProduct() {
  ...
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: async ({ id, product }: { id: ProductId; product: Product }) => {
      if (!actor) throw new Error('Actor not available');
  ...
  return useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: ['cart', identity?.getPrincipal().toString() ?? Principal.anonymous().toString()],
      });
    },
  });
}
...
export function useAddToWishlist() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: ['wishlist', identity?.getPrincipal().toString() ?? Principal.anonymous().toString()],
      });
    },
  });
}
