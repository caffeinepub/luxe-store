    ...
    return (
      <div className="max-w-3xl mx-auto px-4 py-8">
        ...
        <div className="space-y-4">
          {["order-sk-1", "order-sk-2", "order-sk-3"].map((skKey) => (
            <Skeleton key={skKey} className="h-28 w-full rounded-lg" />
          ))}
        </div>
      </div>
    );
      ...
      <div className="space-y-4">
        {sortedOrders.map((order) => (
          <button
            ...
            type="button"
            className="bg-card border border-border rounded-lg p-5 cursor-pointer hover:border-accent/30 transition-all group w-full text-left"
            onClick={() =>
              navigate({
                to: "/orders/$orderId",
